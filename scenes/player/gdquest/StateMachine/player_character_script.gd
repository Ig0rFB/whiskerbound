...
@export var interactAction : String = "interact"
...
@onready var camera_3d = $OrbitView/Camera3D

#particles variables
@onready var movement_dust = %MovementDust
@onready var jump_particles = preload("res://scenes/player/gdquest/VFX/jump_particles.tscn")
@onready var land_particles = preload("res://scenes/player/gdquest/VFX/land_particles.tscn")
...
func handle_interaction():
	...
	if interaction_raycast.is_colliding():
		...
		var interactable_owner = null

		if collider:
			# Check if the object we hit is interactable
			if collider.has_node("Interactable"):
				interactable_owner = collider
			# If not, check if its parent is interactable (for complex models)
			elif collider.get_parent() and collider.get_parent().has_node("Interactable"):
				interactable_owner = collider.get_parent()

		if interactable_owner:
			interactable_node = interactable_owner.get_node("Interactable")
			debug_hud.display_interact_target(interactable_owner.name)
			debug_hud.show_interaction_prompt()
			if Input.is_action_just_pressed(interactAction):
				interactable_node.interact(self)
		else:
			debug_hud.display_interact_target("None")
			debug_hud.hide_interaction_prompt()
	else:
		debug_hud.display_interact_target("None")
		debug_hud.hide_interaction_prompt()
