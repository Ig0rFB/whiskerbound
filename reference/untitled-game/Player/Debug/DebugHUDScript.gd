...
@onready var interact_target_LT = %InteractTargetLT
...
func display_jump_buffer(jump_buffer : bool):
	jump_buffer_LT.set_text("True" if jump_buffer else "")
...
func display_interact_target(target_name : String):
	interact_target_LT.set_text(target_name)
